...
describe("Index Route", () => {
  ...
  describe("Index Component", () => {
    ...
    it("renders main heading", () => {
      ...
      expect(
        screen.getByRole("heading", {
          level: 1,
          name: "Señor Mano te enseña castellano",
        })
      ).toBeInTheDocument();
    });
    ...
    it("renders introduction text", () => {
      ...
      expect(
        screen.getByText(
          /Estos cuadernillos para practicar español están diseñados específicamente/
        )
      ).toBeInTheDocument();
    });
    ...
    it("renders all cuadernillos for present tense", () => {
      ...
      expect(
        screen.getAllByText(/Cuadernillo 1: primera conjugación \(-AR\)/)
      ).toHaveLength(2); // present and past
      expect(
        screen.getAllByText(/Cuadernillo 2: segunda conjugación \(-ER\)/)
      ).toHaveLength(2);
      expect(
        screen.getAllByText(/Cuadernillo 3: tercera conjugación \(-IR\)/)
      ).toHaveLength(2);
      expect(
        screen.getAllByText(/Cuadernillo 4: todos los verbos regulares/)
      ).toHaveLength(2);
    });
    ...
    it("renders preview links for each cuadernillo", () => {
      ...
      expect(
        screen.getAllByRole("link", { name: "👁️ Vista previa (JA)" }).length
      ).toBeGreaterThan(0);
      expect(
        screen.getAllByRole("link", { name: "👁️ Vista previa (EN)" }).length
      ).toBeGreaterThan(0);
    });
    ...
    it("renders download links for each cuadernillo", () => {
      ...
      expect(
        screen.getAllByRole("link", { name: "📖 Descargar (JA)" }).length
      ).toBeGreaterThan(0);
      expect(
        screen.getAllByRole("link", { name: "📖 Descargar (EN)" }).length
      ).toBeGreaterThan(0);
    });
    ...
    it("renders printing information box", () => {
      render(<IndexWrapper />);
      expect(
        screen.getByText("📋 Información de impresión")
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Los PDFs están optimizados para formato DIN A5/)
      ).toBeInTheDocument();
    });
